...
from efdi.domain.models import AfiliadoConVacunas, EstadoExtraccion, Extraccion, Lote
...
from efdi.pdf.parallel_vacunacion import _worker as pdf_worker_vacunacion

log = logging.getLogger(__name__)


def _generar_pdfs_vacunacion(
    ...
    if settings.pdf_workers == 0 or n < settings.pdf_parallel_threshold or pool is None:
        for afiliado, path in tareas:
            ...
            generar_pdf_vacunacion(afiliado, path)
        return n
    ...
    results = list(pool.imap_unordered(pdf_worker_vacunacion, payload, chunksize=chunksize))
    return len(results)


def _procesar_lote_vacunacion(
    job: Extraccion, lote: Lote, pool: "mp.pool.Pool | None" = None,
) -> Lote:
    log.info("vacunacion.lote.start", extra={"job": str(job.id), "lote": lote.numero})
    ...
    try:
        ...
        afiliados = agrupar_por_afiliado_vacunacion(registros)
        ...
        total_pdfs = _generar_pdfs_vacunacion(tareas, pool=pool)
...
def ejecutar_extraccion_vacunacion(job: Extraccion) -> None:
    """Ejecuta el job de Vacunación completo lote a lote."""
    try:
        ...
        def _run_lote(lote: Lote) -> Lote | None:
            ...
            procesado = _procesar_lote_vacunacion(job, lote, pool=pool)
        ...
        store.save(job)
        log.info(
            "vacunacion.extraccion.done",
            extra={"job": str(job.id), "pdfs": total_pdfs},
        )

    except Exception as e:  # noqa: BLE001
        log.exception("vacunacion.extraccion.failed")
        job.estado = EstadoExtraccion.FAILED
