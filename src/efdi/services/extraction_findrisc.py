"""Orquestador FINDRISC: divide en lotes → query → agrupar → PDFs → zip."""
import logging
import math
...
from efdi.domain.models import AfiliadoConFindrisc, EstadoExtraccion, Extraccion, Lote
...
from efdi.pdf.parallel_findrisc import _worker as pdf_worker_findrisc
...
def _procesar_lote_findrisc(job: Extraccion, lote: Lote, pool: "mp.pool.Pool | None" = None) -> Lote:
    log.info("findrisc.lote.start", extra={"job": str(job.id), "lote": lote.numero})
    ...
    try:
        ...
        afiliados = agrupar_por_afiliado_findrisc(registros)
        ...
        lote.completado_en = datetime.now()
        store.save_lote(lote)
        log.info("findrisc.lote.done", extra={"job": str(job.id), "lote": lote.numero, "pdfs": total_pdfs})
        return lote

    except Exception as e:  # noqa: BLE001
        log.exception("findrisc.lote.failed")
...
def ejecutar_extraccion_findrisc(job: Extraccion) -> None:
    """Ejecuta el job FINDRISC completo lote a lote."""
    try:
        ...
        store.save(job)
        log.info("findrisc.extraccion.start", extra={"job": str(job.id), "limite": job.limite, "lotes": n_lotes})
        ...
        def _run_lote(lote: Lote) -> Lote | None:
            ...
            procesado = _procesar_lote_findrisc(job, lote, pool=pool)
        ...
        if lotes_fallidos == n_lotes:
            ...
            job.estado = EstadoExtraccion.COMPLETED
        store.save(job)
        log.info("findrisc.extraccion.done", extra={"job": str(job.id), "pdfs": total_pdfs})

    except Exception as e:  # noqa: BLE001
        log.exception("findrisc.extraccion.failed")
        job.estado = EstadoExtraccion.FAILED
