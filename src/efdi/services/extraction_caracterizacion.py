"""Orquestador Caracterización Familiar: lotes → query sibacom → agrupar por familia → PDFs → zip."""
import logging
import math
...
from efdi.domain.models import EstadoExtraccion, Extraccion, FamiliaCaracterizada, Lote
...
from efdi.pdf.parallel_caracterizacion import _worker as pdf_worker_caracterizacion

log = logging.getLogger(__name__)


def _generar_pdfs_caracterizacion(
    tareas: list[tuple[FamiliaCaracterizada, Path]],
    pool: "mp.pool.Pool | None" = None,
) -> int:
    n = len(tareas)
    if settings.pdf_workers == 0 or n < settings.pdf_parallel_threshold or pool is None:
        for familia, path in tareas:
            path.parent.mkdir(parents=True, exist_ok=True)
            generar_pdf_caracterizacion(familia, path)
        return n
    dirs = {p.parent for _, p in tareas}
    for d in dirs:
        d.mkdir(parents=True, exist_ok=True)
    payload = [(fam, str(p)) for fam, p in tareas]
    chunksize = max(20, n // (pool._processes * 8))  # type: ignore[attr-defined]
    results = list(pool.imap_unordered(pdf_worker_caracterizacion, payload, chunksize=chunksize))
    return len(results)


def _procesar_lote_caracterizacion(job: Extraccion, lote: Lote, pool: "mp.pool.Pool | None" = None) -> Lote:
    log.info("caracterizacion.lote.start", extra={"job": str(job.id), "lote": lote.numero})
    ...
    try:
        ...
        familias = agrupar_por_familia_caracterizacion(registros)
        lote.total_afiliados = len(familias)

        lote.fase = f"Generando PDFs ({len(familias)} familias)…"
        store.save_lote(lote)
        ...
        for familia in familias:
            pdf_path = lote_dir / familia.doc_key / f"{familia.pdf_key}.pdf"
            tareas.append((familia, pdf_path))

        total_pdfs = _generar_pdfs_caracterizacion(tareas, pool=pool)
        ...
        lote.completado_en = datetime.now()
        store.save_lote(lote)
        log.info("caracterizacion.lote.done", extra={"job": str(job.id), "lote": lote.numero, "pdfs": total_pdfs})
        return lote

    except Exception as e:  # noqa: BLE001
        log.exception("caracterizacion.lote.failed")
...
def ejecutar_extraccion_caracterizacion(job: Extraccion) -> None:
    """Ejecuta el job de Caracterización Familiar completo lote a lote."""
    try:
        ...
        store.save(job)
        log.info("caracterizacion.extraccion.start", extra={"job": str(job.id), "limite": job.limite, "lotes": n_lotes})
        ...
        def _run_lote(lote: Lote) -> Lote | None:
            ...
            procesado = _procesar_lote_caracterizacion(job, lote, pool=pool)
        ...
        if lotes_fallidos == n_lotes:
            ...
            job.estado = EstadoExtraccion.COMPLETED
        store.save(job)
        log.info("caracterizacion.extraccion.done", extra={"job": str(job.id), "pdfs": total_pdfs})

    except Exception as e:  # noqa: BLE001
        log.exception("caracterizacion.extraccion.failed")
        job.estado = EstadoExtraccion.FAILED
