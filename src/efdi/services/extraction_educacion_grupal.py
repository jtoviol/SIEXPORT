"""Orquestador Educación Grupal: divide en lotes → query → agrupar → PDFs → zip."""
import logging
import math
...
from efdi.domain.models import AfiliadoConEducacionGrupal, EstadoExtraccion, Extraccion, Lote
from efdi.domain.services import agrupar_por_afiliado_educacion_grupal
...
from efdi.pdf.parallel_educacion_grupal import _worker as pdf_worker_eg

log = logging.getLogger(__name__)


def _generar_pdfs_eg(
    ...
    if settings.pdf_workers == 0 or n < settings.pdf_parallel_threshold or pool is None:
        for afiliado, path in tareas:
            ...
            generar_pdf_educacion_grupal(afiliado, path)
        return n
    dirs = {p.parent for _, p in tareas}
    for d in dirs:
        d.mkdir(parents=True, exist_ok=True)
    payload = [(af, str(p), None) for af, p in tareas]
    chunksize = max(20, n // (pool._processes * 8))
    results = list(pool.imap_unordered(pdf_worker_eg, payload, chunksize=chunksize))
    return len(results)


def _procesar_lote_eg(job: Extraccion, lote: Lote, pool: "mp.pool.Pool | None" = None) -> Lote:
    log.info("eg.lote.start", extra={"job": str(job.id), "lote": lote.numero})
    ...
    try:
        ...
        registros = repo.obtener_registros(
            ...
            limite=lote.tamano, offset=lote.offset_inicio,
            regimen=job.regimen,
        )
        ...
        afiliados = agrupar_por_afiliado_educacion_grupal(registros)
        ...
        total_pdfs = _generar_pdfs_eg(tareas, pool=pool)
        ...
        lote.completado_en = datetime.now()
        store.save_lote(lote)
        log.info("eg.lote.done", extra={"job": str(job.id), "lote": lote.numero, "pdfs": total_pdfs})
        return lote

    except Exception as e:
        log.exception("eg.lote.failed")
...
def ejecutar_extraccion_educacion_grupal(job: Extraccion) -> None:
    try:
        ...
        store.save(job)
        log.info("eg.extraccion.start", extra={"job": str(job.id), "limite": job.limite, "lotes": n_lotes})
        ...
        def _run_lote(lote: Lote) -> Lote | None:
            ...
            procesado = _procesar_lote_eg(job, lote, pool=pool)
        ...
        if lotes_fallidos == n_lotes:
            ...
            job.estado = EstadoExtraccion.COMPLETED
        store.save(job)
        log.info("eg.extraccion.done", extra={"job": str(job.id), "pdfs": total_pdfs})

    except Exception as e:
        log.exception("eg.extraccion.failed")
        job.estado = EstadoExtraccion.FAILED
