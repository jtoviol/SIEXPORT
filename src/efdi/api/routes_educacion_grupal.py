"""Endpoints REST para el módulo Educación Grupal."""
import math
import shutil
...
from efdi.services.extraction_educacion_grupal import ejecutar_extraccion_educacion_grupal

router = APIRouter(
    prefix="/educacion-grupal",
    tags=["educacion-grupal"],
    dependencies=[Depends(require_modulo("educacion-grupal"))],
)
...
@router.get(
    "/extractions/count",
    summary="Conteo previo de registros Educación Grupal para un rango de fechas",
)
async def contar_registros_educacion_grupal(
    desde: date = Query(...),
    hasta: date = Query(...),
    ...
    if regimen:
        ...
        regimen = r
    repo = get_educacion_grupal_repository()
...
@router.post(
    ...
    summary="Crear extracción de PDFs Educación Grupal",
    dependencies=[Depends(require_no_viewer)],
)
async def crear_extraccion_educacion_grupal(
    ...
    if limite is None:
        repo = get_educacion_grupal_repository()
        total = repo.get_total(req.desde, req.hasta, regimen=regimen)
        if total <= 0:
            raise HTTPException(
                status_code=400,
                detail="No se encontraron registros de Educación Grupal para el rango indicado.",
            )
        limite = total

    tamano_lote = req.tamano_lote or _auto_tamano_lote(limite)

    nombre_base = f"Educación Grupal {req.desde}—{req.hasta}"
    if regimen:
    ...
    job = Extraccion(
        ...
        tipo=ExtraccionTipo.EDUCACION_GRUPAL,
    ...
    store.save(job)
    background.add_task(ejecutar_extraccion_educacion_grupal, job)
...
@router.get(
    ...
    summary="Listar extracciones Educación Grupal",
)
async def listar_extracciones_educacion_grupal() -> list[ExtraccionResp]:
    return [
        ExtraccionResp(**j.model_dump())
        for j in store.list_by_tipo(ExtraccionTipo.EDUCACION_GRUPAL)
    ]
...
@router.get(
    "/extractions/{job_id}",
    response_model=ExtraccionResp,
    summary="Estado de una extracción Educación Grupal",
)
async def obtener_extraccion_educacion_grupal(job_id: UUID) -> ExtraccionResp:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
    return ExtraccionResp(**job.model_dump())
...
@router.get(
    ...
    summary="Listar lotes de una extracción Educación Grupal",
)
async def listar_lotes_educacion_grupal(job_id: UUID) -> list[Lote]:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
...
@router.get(
    ...
    response_model=Lote,
    summary="Estado de un lote Educación Grupal",
)
async def obtener_lote_educacion_grupal(job_id: UUID, numero: int) -> Lote:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
...
@router.get(
    ...
    summary="Descargar ZIP de un lote Educación Grupal",
    response_class=FileResponse,
)
async def descargar_lote_educacion_grupal(job_id: UUID, numero: int) -> FileResponse:
    lote = store.get_lote(job_id, numero)
    if lote is None:
        raise HTTPException(status_code=404, detail=f"Lote {numero} no existe")
    ...
    job = store.get(job_id)
    base = safe_filename(
        job.nombre if job else None,
        f"educacion_grupal_lote_{numero:03d}_{job_id}",
    )
...
@router.patch(
    ...
    response_model=ExtraccionResp,
    summary="Renombrar una extracción Educación Grupal",
    dependencies=[Depends(require_no_viewer)],
)
async def renombrar_extraccion_educacion_grupal(job_id: UUID, req: RenombrarJobReq) -> ExtraccionResp:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
...
@router.post(
    ...
    summary="Cancelar extracción Educación Grupal en curso",
    dependencies=[Depends(require_no_viewer)],
)
async def cancelar_extraccion_educacion_grupal(job_id: UUID) -> dict:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
...
@router.delete(
    "/extractions/{job_id}",
    summary="Eliminar extracción Educación Grupal",
    dependencies=[Depends(require_no_viewer)],
)
async def eliminar_extraccion_educacion_grupal(job_id: UUID) -> dict:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
...
@router.get(
    ...
    summary="Mega-ZIP con todos los lotes Educación Grupal",
    response_class=FileResponse,
)
async def descargar_extraccion_educacion_grupal(job_id: UUID) -> FileResponse:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
    if job.estado != EstadoExtraccion.COMPLETED:
        raise HTTPException(
            status_code=409,
            detail=f"Extracción en estado '{estado_label(job.estado)}'",
        )

    if not job.zip_path or not Path(job.zip_path).exists():
        lotes = store.list_lotes(job_id)
        zips_lotes = [
            Path(l.zip_path) for l in lotes
            if l.zip_path and Path(l.zip_path).exists()
        ]
        if not zips_lotes:
    ...
    return FileResponse(
        path=job.zip_path,
        filename=f"{safe_filename(job.nombre, f'educacion_grupal_{job_id}')}.zip",
        media_type="application/zip",
    )
...
@router.get(
    ...
    summary="Árbol de archivos de una extracción Educación Grupal",
)
async def listar_archivos_educacion_grupal(job_id: UUID) -> dict:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
    if job.estado != EstadoExtraccion.COMPLETED:
...
@router.get(
    ...
    summary="Descargar PDF individual Educación Grupal",
    response_class=FileResponse,
)
async def descargar_pdf_educacion_grupal(job_id: UUID, afiliado: str, filename: str) -> FileResponse:
    job = store.get(job_id)
    if job is None or job.tipo != ExtraccionTipo.EDUCACION_GRUPAL:
        raise HTTPException(status_code=404, detail="Extracción Educación Grupal no encontrada")
